    ...
    def handle(msg: Any): XBeeMessageDistributor @processCps = msg match {
      case XBeeDataPacket(_, from, _, _, payload) => payload match {
              case packet @ AnnounceService(_, _) =>
                log.trace("Passadi distributor got an announce: {}", packet)
              ...
              case other =>
                log.trace("Passadi distributor does not forward packet with content {}", other)
                this //do not forward
      }
      case end: ProcessEnd =>
        end match {
      ...
      case other =>
        //ignore
        log.debug("Passadi distributor is ignoring message {}", other)
        this
    }
  }
